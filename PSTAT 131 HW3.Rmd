...
Also create a heatmap-style correlation plot for better interpretation (as this includes numbers): 

```{r}
# Create a heatmap-style correlation plot that can allow for better interpretation 
cor_titanic_train %>%
  stretch() %>%
...
Note: There was also some negligible correlations (very light in color on the correlation matrix, had a value very close to 0 in the heatmap-style correlation plot). 
...
Answer(Question 9): The logistic regression model for classification using the "glm" engine acieved the highest accuracy on the training data (Log Reg Acc = 0.8160112). 
...
Answer(Question 10): The training accuracy was 0.8160112 while the testing accuracy was 0.8156425. Note the values do differ, the training accuracy is slightly higher than the testing accuracy. I think this is the case because we selected the model (logistic regression) based on it's accuracy on the training dataset rather than its accuracy on the (unseen) test dataset. 
